...
tools: ['search/codebase', 'read', 'edit', 'execute']
...
- Handoff format:
  ...
  handoffs:
    - label: 'Button Text'
      agent: Target Agent Name # Must be the target's name field, NOT the filename slug
      prompt: 'Context for the target agent'
      send: false
  ```
- **Handoff target rules**: the `agent` value must exactly match the target's `name` frontmatter field. The target agent must NOT have `user-invocable: false` or `disable-model-invocation: true`, as either setting prevents it from being a valid handoff target.
...
3. **Cross-references**: Every `agent` value in handoffs must exactly match the target `.agent.md` file's `name` frontmatter field (not the filename). The target agent must not have `user-invocable: false` or `disable-model-invocation: true` — either setting prevents it from being a valid handoff target
